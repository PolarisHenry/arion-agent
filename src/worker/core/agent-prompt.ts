...
function buildToolDiscipline(): string {
  return [
    ...
    '- **主动记下可复用的稳定信息。** 当用户给出会反复用到的信息（资源位置/ID、偏好、常用对象/联系人），调 `memory` 工具 save 记下来；下次别再问。不要记琐碎或仅本次有效的临时信息。',
    '- **保持记忆准确：更新与清理。** 当用户告知信息变更，或你发现已记的信息与本轮最新事实矛盾、已过时，立即处理：更新用 `memory` save 并复用原 key 覆盖（不要另起新 key 留下过期重复项），作废用 `memory` delete 清理。给有时效的信息（当前 sprint、临时事务等）存时设 expiresAt，到期系统自动忽略，无需手动删。'
  ].join('\n');
}
