...
import { useInfobarTranslation } from '@/hooks/use-infobar-translation';
    ...
    <Infobar {...props}>
      <InfobarHeader className='bg-sidebar sticky top-0 z-10 flex flex-row items-center justify-between gap-2 border-b px-4 py-3'>
        <div className='min-w-0 flex-1'>
          <h2 className='text-lg font-semibold wrap-break-word'>{t(data.title)}</h2>
        </div>
              ...
              {data.sections && data.sections.length > 0 ? (
                data.sections.map((section) => (
                  <div key={section.title} className='flex flex-col gap-3'>
                    {section.title && (
                      <h3 className='text-foreground text-sm font-semibold'>{t(section.title)}</h3>
                    )}
                    {section.description && (
