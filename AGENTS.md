...
- **Authentication**: better-auth (email/password + organizations)
...
- better-auth for authentication (email/password + master-sub-account model)
- Master-sub-account model with RBAC for multi-tenant access
- Permission-based navigation visibility (client-side UX filtering)
...
BETTER_AUTH_URL=http://localhost:3000  # Base URL for callbacks and redirects
```

### Required for Database (PostgreSQL)

```env
DATABASE_URL=postgres://arion:arion_dev@localhost:5432/arion_agent
```
...
**Note**: Sentry is disabled by default (`NEXT_PUBLIC_SENTRY_DISABLED=true`).
    ...
    items: [
      {
        ...
        shortcut: ['d', 'd'],
        items: [],
        access: { permission: 'menu:dashboard' } // RBAC check
      }
    ]
  }
];
...
- `permission: string` - Requires specific permission (checked via `/api/auth/me`)

### Client-Side Filtering

Navigation is filtered client-side using `useFilteredNavGroups()` from `src/hooks/use-nav.ts`. This hook calls `useMe()` (fetches user + permissions from `/api/auth/me`) and filters nav items based on the user's resolved permissions. This is for UX only — actual security checks must happen server-side via the API layer.
...
Dashboard routes use better-auth session checks. Pages that require authentication:
...
Server-side permission checks use the resolved permissions from `auth.api.getSession()`:
...
export default async function Page() {
  ...
  const hasAccess = permissions.includes('manage:products');
  // ...
}
```

Client-side permission info is available via `useMe()` from `@/hooks/use-me`, which fetches user and permissions from `/api/auth/me`.
